...
class EtiketUygulamasi:
    def __init__(self, root):
        ...
        updatemenu.add_command(label="Son Değişiklikler", command=self.show_changelog)
        menubar.add_cascade(label="Güncelleme", menu=updatemenu)
        self.root.config(menu=menubar)
        # Menü barının hemen altına versiyon numarası ekle
        self.version_label = tk.Label(self.root, text="Sürüm: 22.07.24.001", font=("Arial", 9), bg=BG_COLOR, fg="#888", anchor="e")
    ...
    def build_step(self):
        ...
        # Üstte stepper ve nav_frame (butonlar)
    ...
    def show_help(self):
        self.show_info("Kullanım Kılavuzu", "1. Turnuva bilgilerini girin.\n2. Kategorileri ve ödülleri ekleyin.\n3. Etiket ayarlarını yapın.\n4. PDF oluşturun veya önizleyin.\n\nHer adımda ileri/geri ile gezebilirsiniz.\nSorularınız için: ertugrul.yazilim@gmail.com")
    ...
    def guncelleme_kontrol_et(self):
        # Basit örnek: Sürüm kontrolü (gerçek uygulamada web'den çekilebilir)
        GUNCEL_SURUM = "22.07.24.002"  # Örnek: sunucudan çekilecek
        MEVCUT_SURUM = "22.07.24.001"
        if MEVCUT_SURUM == GUNCEL_SURUM:
            self.show_info("Güncelleme", "Zaten en güncel sürümü kullanıyorsunuz.")
        else:
            def indir():
                self.show_info("Güncelleme", "Yeni sürüm indiriliyor... (Simülasyon)")
            def devam():
                self.show_info("Güncelleme", "Mevcut sürümle devam ediliyor.")
            popup = tk.Toplevel(self.root)
            popup.title("Güncelleme Mevcut")
            popup.geometry("380x180")
            popup.configure(bg=BG_COLOR)
            tk.Label(popup, text=f"Yeni sürüm bulundu: {GUNCEL_SURUM}\nİndirmek ve kurmak ister misiniz?", font=("Arial", 11), bg=BG_COLOR, wraplength=340, justify="left").pack(pady=18)
            btn1 = self.styled_button(popup, text="Evet, indir ve kur", command=lambda: [indir(), popup.destroy()], font=("Arial", 9, "bold"))
            btn1.pack(side=tk.LEFT, padx=18, pady=8)
            btn2 = self.styled_button(popup, text="Hayır, mevcut sürümü kullan", command=lambda: [devam(), popup.destroy()], font=("Arial", 9, "bold"), bg="#d32f2f", activebackground="#b71c1c")
            btn2.pack(side=tk.RIGHT, padx=18, pady=8)
            popup.transient(self.root)
            popup.grab_set()

    def show_changelog(self):
        # Basit changelog örneği (ileride dosyadan da okunabilir)
        changelog = (
            "22.07.24.002\n- PDF çıktısı iyileştirildi\n- Küçük hata düzeltmeleri\n\n"
            "22.07.24.001\n- İlk yayınlanan sürüm\n- Menü barı ve Hakkında penceresi\n"
        )
        top = tk.Toplevel(self.root)
        top.title("Son Değişiklikler")
        top.geometry("420x320")
        top.configure(bg=BG_COLOR)
        tk.Label(top, text="Son Değişiklikler (Changelog)", font=("Arial", 13, "bold"), bg=BG_COLOR).pack(pady=(16, 8))
        text = tk.Text(top, font=("Consolas", 10), bg="#f7f7f7", fg="#222", wrap="word", height=12, borderwidth=0)
